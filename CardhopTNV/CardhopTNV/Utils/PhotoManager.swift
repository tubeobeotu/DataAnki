...
//        }
        ...
        let actionSheetController: UIAlertController = UIAlertController(title: "Contact Picture", message: nil, preferredStyle: .alert)
        let cancelAction: UIAlertAction = UIAlertAction(title: "Cancel", style: .default) { action -> Void in
            //Just dismiss the action sheet
        }
        let takePhoto: UIAlertAction = UIAlertAction(title: "Take Picture", style: .default) { action -> Void in
            self.delegate?.presentVC().present(self.imageTakePictureController!, animated: true, completion: nil)
        }
        let selecFromLib: UIAlertAction = UIAlertAction(title: "Choose Picture", style: .default) { action -> Void in
